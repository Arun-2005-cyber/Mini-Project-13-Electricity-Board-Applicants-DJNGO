...
function Home() {
  ...
  useEffect(() => {
    fetchData()
  }, [currentPage, startDate, endtDate, searchQuery])

  const fetchData = async () => {
    try {
      let url = `http://127.0.0.1:8000/api/getApplicantsData/?page=${currentPage}`;
      if (startDate && endtDate) {
        url += `&start_date=${startDate.toISOString().split("T")[0]}&end_date=${endtDate.toISOString().split("T")[0]
          }`;
      }
    ...
    <>
      <div className="container mt-3">
        ...
        <Row className='mb-3'>
          ...
          <Col md={2}>
            <Datepicker selected={startDate} className="form-control date" onChange={handleStartDateChange} placeholderText="From Date" />
          </Col>
          <Col md={2}>
            <Datepicker selected={endtDate} className="form-control date" onChange={handleEndDateChange} placeholderText="To Date" />
          </Col>

          <Col md={3}></Col>
          <Col md={5}>
            <input type="text" className='form-control' value={searchQuery} onChange={handleSearchChange} placeholder='Search by applicant ID' />
          </Col>
        </Row>
